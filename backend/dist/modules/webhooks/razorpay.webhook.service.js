...
export function verifyRazorpaySignature(rawBody, signatureHeader) {
 ...
 * ──────────────────────────────────────────────────────────── */
export async function processRazorpayWebhookEvent(payload) {
    const eventId = payload.id; // Razorpay's unique event id, e.g. "evt_..."
    ...
    const razorpayOrderId = paymentEntity?.order_id ?? refundEntity?.notes?.razorpayOrderId;
    if (!razorpayOrderId) {
    ...
    switch (eventType) {
        case "payment.captured": {
            ...
            try {
                ...
                console.error("[razorpay-webhook] pushToShiprocket failed after payment.captured", err);
            }
...
//# sourceMappingURL=razorpay.webhook.service.js.map