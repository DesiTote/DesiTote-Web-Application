...
import { ORDER_STATUS_BUCKET_MAP, ORDER_STATUS_BUCKET_ORDER, REVENUE_EXCLUDED_STATUSES, } from "./dashboard.constant.js";
/* ────────────────────────────────────────────────────────────
 ...
 * ──────────────────────────────────────────────────────────── */
export async function fetchSalesOverview(range) {
...
export async function fetchOrderStatusBreakdown() {
    ...
    const bucketCounts = Object.fromEntries(ORDER_STATUS_BUCKET_ORDER.map((bucket) => [bucket, 0]));
    for (const row of raw) {
        const bucket = ORDER_STATUS_BUCKET_MAP[row._id];
        if (bucket)
