...
export const createProductService = async (payload, files) => {
    ...
    try {
        ...
        uploadedImageUrls = await Promise.all(files.map((file) => uploadFileToS3(file, "catalog-products")));
...
export const updateProductByIdService = async (productId, body, newUploadedFiles) => {
    ...
    const targetsForS3Deletion = currentProduct.images.filter((oldUrl) => !retainedS3Urls.includes(oldUrl));
    // 3. Purge those removed items from your S3 bucket
    if (targetsForS3Deletion.length > 0) {
        Promise.all(targetsForS3Deletion.map(deleteFileFromS3)).catch((err) => console.error("[S3-BACKGROUND-PURGE-FAIL]: Failed to erase orphaned images", err));
    }
    if (targetsForS3Deletion.length > 0) {
...
export const archiveProductService = async (productId) => {
    ...
    const product = await Product.findById(productId).select("title sku stock lowStockThreshold price status productCategory thumbnail");
    if (!product) {
        throw new ApiError(404, "Product record not found");
    }
