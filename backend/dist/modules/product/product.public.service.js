...
import { getCachedProducts, setCachedProducts, buildProductsCacheKey, } from "../../utils/productCache.js";
...
const parseCategorySelection = (raw) => {
    if (!raw)
...
export const getPublicProductsService = async (query) => {
    ...
    if (limit > MAX_LIMIT)
        limit = MAX_LIMIT;
    const { categories, isAllSelected } = parseCategorySelection(query.category);
    ...
    const totalProducts = result?.totalCount?.length > 0
...
export const getPublicProductBySlugService = async (slug) => {
    ...
    const product = await Product.findOne({
        ...
        .select("title slug shortDescription description thumbnail images price discountPrice productCategory tags color material weight dimensions stock averageRating reviewCount")
        .lean();
    if (!product) {
...
export async function fetchStorefrontBestsellers(limit) {
    const query = {
        limit,
        type: "bestSeller",
    };
...
//# sourceMappingURL=product.public.service.js.map