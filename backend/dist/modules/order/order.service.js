...
async function generateOrderNumber() {
    ...
    const counter = await Counter.findByIdAndUpdate(`order-${year}`, { $inc: { seq: 1 } }, { new: true, upsert: true });
...
export async function getRecentOrdersService(userId, limit = 10) {
    validateObjectId(userId, "userId");
...
export async function cancelOrderService(input) {
    ...
    if (!isOrderCancellable(order.status)) {
        throw new ApiError(409, order.status === "shipped" ||
...
export async function createRazorpayOrderService(input) {
    ...
    if (existingOrder) {
        ...
        order = await Order.findByIdAndUpdate(existingOrder._id, {
...
export async function verifyRazorpayPaymentService(input) {
    ...
    const paidOrder = await Order.findByIdAndUpdate(order._id, {
        "payment.status": "paid",
...
export async function pushToShiprocket(order, sessionId) {
    const claimed = await Order.findOneAndUpdate({ _id: order._id, "shiprocket.status": { $in: ["pending", "failed"] } }, { "shiprocket.status": "pushing" }, { new: true });
    if (!claimed) {
