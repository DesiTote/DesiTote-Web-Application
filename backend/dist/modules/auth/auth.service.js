...
import { EMAIL_SUBJECTS, OTP_EMAIL_CONTENT } from "../../constants/customer/email.js";
...
export const registerUser = async (data) => {
    ...
    if (existingUser)
        throw new ApiError(401, "We couldn't complete your registration. Please try again.");
...
export const loginUser = async (data) => {
    ...
    const failData = await redis.hgetall(loginFailKey);
...
export const requestActionOtp = async (email, type) => {
    if (!email || !type)
        throw new ApiError(400, "Please provide a valid email address and otp type.");
    const redis = getRedis();
    const hashKey = `auth:${type}:${email}`;
    const verifiedKey = `verified:${type}:${email}`;
    const isAlreadyVerified = await redis.get(verifiedKey);
    ...
    const authData = await redis.hgetall(hashKey);
...
export const processOtpVerification = async (email, otp, type, successExpiry = 600) => {
    if (!email || !type || !otp) {
