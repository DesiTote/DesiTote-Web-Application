...
export async function fetchRevenueTrend(range: TrendRange) {
...
export async function fetchAverageOrderValue() {
    ...
    const trend: { label: string; aov: number }[] = [];
    ...
    const overallAov =
        overall && overall.totalOrders > 0 ? Math.round(overall.totalRevenue / overall.totalOrders) : 0;
...
export async function fetchBestSellingProducts(sortBy: "quantity" | "revenue", limit: number) {
...
export async function fetchNewSignupsTrend() {
    ...
    const result: { label: string; signups: number }[] = [];
    for (let i = 0; i < months; i++) {
        const d = new Date(startDate);
        d.setMonth(d.getMonth() + i);
        const year = d.getFullYear();
...
export async function fetchMostWishlistedProducts(limit: number) {
